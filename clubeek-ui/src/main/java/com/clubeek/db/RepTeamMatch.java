...
public class RepTeamMatch implements Repository<TeamMatch> {
	...
	public enum TableColumn {
		ID("id"), START("start"), SCORE_POS("score_pos"), SCORE_NEG("score_neg"), HOME_COURT("home_court"), COMMENT("comment"), PUBLISH(
				"publish"), CLUB_TEAM_ID("club_team_id"), CLUB_RIVAL_ID("club_rival_id"), CLUB_RIVAL_COMMENT("club_rival_comment");
	...
	public void readValue(ResultSet result, int resultsColumnId, TeamMatch data, Object dataColumnId) throws SQLException {
		...
		case SCORE_NEG:
			...
			break;
		case HOME_COURT:
			data.setHomeCourt(result.getInt(resultsColumnId) == 0 ? false : true);
			break;
		case PUBLISH:
			data.setPublish(result.getInt(resultsColumnId) == 0 ? false : true);
			break;
		case COMMENT:
	...
	private static RepTeamMatch gameDb = new RepTeamMatch();
